...
class Client {
  ...
  bind(dn, password, callback) {
    ...
    if ('function' == typeof dn || 'undefined' == typeof dn) {
      callback = dn;

      if (self.config.bindDN && self.config.bindCredentials) {
        self.bindType = 'defaultCredentials';
        dn = self.config.bindDN;
        password = self.config.bindCredentials;

      } else {
        self.bindType = 'anonymous';
        dn = '';
        password = ''
      }
